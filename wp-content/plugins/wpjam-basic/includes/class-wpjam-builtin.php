...
class WPJAM_Builtin_Page{
	public static function load($current_screen){
		do_action('wpjam_builtin_page_load', $current_screen->base, $current_screen);

		if(!wp_doing_ajax() && wpjam_get_page_summary()){
			add_filter('wpjam_html', [self::class, 'append_page_summary']);
		}

		$method	= 'load_'.str_replace('-', '_', $current_screen->base).'_page';

		if(method_exists(self::class, $method)){
			call_user_func([self::class, $method], $current_screen);
		}
	}

	public static function load_post_page($current_screen){
		$edit_form_hook	= $current_screen->post_type == 'page' ? 'edit_page_form' : 'edit_form_advanced';
	...
	public static function load_term_page($current_screen){
		add_action($current_screen->taxonomy.'_edit_form_fields',	['WPJAM_Term_Option', 'on_edit_form_fields']);
	}

	public static function load_edit_page($current_screen){
	...
	public static function load_upload_page($current_screen){
		...
		self::load_edit_page($current_screen);
	}

	public static function load_edit_tags_page($current_screen){
		...
		if($tax_obj){
			...
			if(wp_doing_ajax()){
				...
				add_action($taxonomy.'_add_form_fields', 	['WPJAM_Term_Option', 'on_add_form_fields']);
			}
	...
	public static function load_users_page($current_screen){
		$GLOBALS['wpjam_list_table']	= new WPJAM_Users_List_Table();
	}

	public static function append_page_summary($html){
...
class WPJAM_Builtin_List_Table extends WPJAM_List_Table{
	...
	public function call_model_list_action($id, $data, $list_action){
		if($fields = $this->get_fields($list_action, $id, ['include_prev'=>true])){
			$defaults	= (new WPJAM_Fields($fields))->get_defaults();
		}else{
			$defaults	= null;
		}
		...
		foreach($ids as $id){
			$result	= call_user_func([$this->model, 'update_meta'], $id, $data, $defaults);
...
class WPJAM_Posts_List_Table extends WPJAM_Builtin_List_Table{
	...
	public function filter_map_meta_cap($caps, $cap, $user_id, $args){
		if($cap == 'edit_post' && empty($args[0])){
			$ptype	= $GLOBALS['current_screen']->post_type;
	...
	public function list_table(){
		...
		if($this->post_type == 'attachment'){
			...
			echo wpjam_get_field_html(['key'=>'post_type',		'type'=>'hidden',	'class'=>'post_type_page',		'value'=>$this->post_type]);
...
class WPJAM_Terms_List_Table extends WPJAM_Builtin_List_Table{
	public function __construct($args=[]){
		...
		if($tax_obj->hierarchical){
			if($tax_obj->sortable){
				$args['sortable']	= [
					'items'			=> $this->get_sorteable_items($taxonomy),
	...
	public function list_table(){
		...
		if($tax_obj->hierarchical && $tax_obj->sortable){
			$sortable_items	= 'data-sortable_items="'.$this->get_sorteable_items($this->taxonomy).'"';
		}else{
			$sortable_items	= '';
		}
	...
	public function get_sorteable_items($taxonomy=''){
		$parent	= $this->get_parent($taxonomy);
		$level	= $parent ? (wpjam_get_term_level($parent)+1) : 0;

		return 'tr.level-'.$level;
	}

	public function get_parent($taxonomy=''){
	...
	public function append_parent_button($html){
		...
		if($tax_obj->hierarchical && isset($tax_obj->levels) && $tax_obj->levels != 1){
			$parent	= $this->get_parent($this->taxonomy);
