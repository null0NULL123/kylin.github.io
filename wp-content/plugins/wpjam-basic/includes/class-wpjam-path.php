...
class WPJAM_Path{
	...
	public function add_type($type, $item){
		...
		if(isset($item['group']) && is_array($item['group'])){
			if(isset($item['group']['key'], $item['group']['title'])){
				...
				self::$groups[$group_key]	= $group_title;
			}else{
				unset($item['group']);
			}
		}
	...
	private function get_taxonomy_path($args){
		...
		if($term_id	= (int)wpjam_array_pull($args, $query_key)){
			if($args['path_type'] == 'template'){
	...
	public static function parse_page_key_options(&$options, $page_key, $path_obj){
		if($path_obj->tabbar
			|| (($group_key = $path_obj->group) && isset(self::$groups[$group_key]))
		){
			if(($group_key = $path_obj->group) && isset(self::$groups[$group_key])){
				$group_title	= self::$groups[$group_key];
				$group_key		= 'group_'.$group_key;
			}else{
				$group_title	= '菜单栏	/常用';
				$group_key		= 'group_tabbar';
			}
			...
			$options[$group_key]['options'][$page_key]	= $path_obj->title;
		}else{
			$options[$page_key]	= $path_obj->title;
		}
	}
	...
	public static function get_path_fields($path_types, $for=''){
		...
		foreach(self::get_registereds() as $page_key => $path_obj){
			...
			self::parse_page_key_options($page_key_fields['page_key']['options'], $page_key, $path_obj);
			...
			if($backup_required){
				if($path_obj->has($path_types, 'AND')){
					if(
						...
						self::parse_page_key_options($backup_fields['page_key_backup']['options'], $page_key, $path_obj);
					}
		...
		if($for == 'qrcode'){
			...
			$page_key_fields['page_key']['options']['group_tabbar']['options']['none']	= '只展示不跳转';
			...
			if($backup_required){
				...
				$backup_fields['page_key_backup']['options']['group_tabbar']['options']['none']	= '只展示不跳转';
