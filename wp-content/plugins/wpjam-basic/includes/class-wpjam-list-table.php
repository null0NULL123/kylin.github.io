...
class WPJAM_List_Table extends WP_List_Table{
	...
	public function __construct($args=[]){
		...
		if($args['columns'] 
			&& (empty($args['primary_column']) 
				|| !isset($args['columns'][$args['primary_column']])
			)
		){
			$args['primary_column']	= array_keys($args['columns'])[0];
		}
	...
	public function parse_args($args){
		...
		foreach(wpjam_list_sort($actions) as $key => $object){
			$object->default_callback	= [$this, 'call_model_list_action'];
		...
		foreach(wpjam_list_sort($column_fields) as $object){
			$column_name	= $object->name;

			$object->filterable	= $object->filterable ?? $this->is_filterable_column($column_name);
	...
	public function get_filter_link($filters, $title, $class=''){
		...
		if($this->query_args){
			foreach($this->query_args as $query_arg){
	...
	public function ajax_response(){
		...
		if($action_type == 'form'){
			...
			if($object->response == 'form'){
				...
				$submit_name	= wpjam_get_parameter('submit_name',	['method'=>'POST']);
	...
	public function ajax_form($list_action, $args=[]){
		...
		if(!empty($object->next) && $object->response == 'form'){
			$submit_button	.= get_submit_button('下一步', 'primary', 'next', false);
		}elseif($submit_text = $object->get_submit_text($id)){
			foreach($submit_text as $submit_key => $submit_item){
...
class WPJAM_List_Table_Action{
	...
	public function callback($id=0, $data=null, $submit_name=''){
		...
		if($callback){
			...
			$result	= call_user_func($this->default_callback, $id, $data, $this->name);
		}

		return is_null($result) ? new WP_Error('null_list_action', '操作回调函数没有正确返回') : $result;
	}

	public function get_row_action($args=[]){
		$args	= wp_parse_args($args, ['id'=>0, 'data'=>[], 'class'=>'', 'style'=>'', 'title'=>'', 'fallback'=>false, 'layout'=>'']);
