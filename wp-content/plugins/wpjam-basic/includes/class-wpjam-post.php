...
class WPJAM_Post{
	...
	public function parse_for_json($args=[]){
		...
		foreach(WPJAM_Post_Option::get_registereds() as $po_obj){
			if($po_obj->is_available_for_post_type($this->post_type)){
				$json	= array_merge($json, $po_obj->prepare_value($this->id));
			}
		}
	...
	public static function get_by_ids($post_ids, $args=[]){
		...
		if($args){
			foreach(['update_post_term_cache', 'update_post_meta_cache'] as $key){
				$cache_args[$key]	= wpjam_array_pull($args, $key);
			}
		}

		if($posts = self::update_caches($post_ids, $cache_args)){
			if($args){
				$posts	= wpjam_list_filter($posts, $args);
			}
		}
...
class WPJAM_Post_Option{
	...
	public function get_fields_object($post_id=null){
	...
	public function prepare_value($post_id){
		if($this->update_callback){
			return [];
		}

		$fields_object	= $this->get_fields_object($post_id);

		return $fields_object->prepare([
			'value_callback'	=> ['WPJAM_Post', 'value_callback'],
			'id'				=> $post_id
		]);
	}
	...
	public static function on_add_meta_boxes($post_type, $post){
		...
		foreach(self::get_registereds() as $name => $object){
			if($object->is_available_for_post_type($post_type) && $object->list_table !== 'only' && $object->title){
				$callback	= $object->meta_box_cb ?: [self::class, 'meta_box_cb'];
	...
	public static function meta_box_cb($post, $meta_box){
		$object	= self::get($meta_box['id']);
	...
	public static function on_after_insert_post($post_id, $post){
		...
		foreach(self::get_registereds() as $name => $object){
			if(!$object->is_available_for_post_type($post_type) || $object->list_table === 'only'){
				continue;
			}
			...
			if($callback = $object->callback ?: $object->update_callback){
				if(is_callable($callback)){
					$result	= call_user_func($callback, $post_id, $data, $fields);
		...
		foreach($post_data as $key => $value){
			if(empty($value)){
				...
				if(empty($custom[$key]) || maybe_unserialize($custom[$key][0]) != $value){
					wpjam_update_metadata('post', $post_id, $key, $value);
				}
			}
		}
	}

	public static function register_list_table_action($post_type){
		$screen_base	= get_current_screen()->base;

		foreach(self::get_registereds() as $name => $object){
			if($object->title 
				&& $object->is_available_for_post_type($post_type) 
				&& ($object->list_table || !in_array($screen_base, ['edit', 'upload']))
			){
