...
class WPJAM_DB{
	...
	public function __call($method, $args){
		if(in_array($method, [
			...
			if(str_starts_with($method, 'cache_get')){
				return wp_cache_get($key, $group);
			}elseif(str_starts_with($method, 'cache_delete')){
				return wp_cache_delete($key, $group);
			}else{
				$data	= $args[1];
				$time	= !empty($args[2]) ? (int)$args[2] : $this->cache_time;

				if(str_starts_with($method, 'cache_add')){
	...
	public function update_caches($ids, $primary=false){
		...
		if(function_exists('wp_cache_get_multiple')){
			...
			foreach($ids as $id){
				$method	= $primary ? 'cache_get_by_primary_key' : 'cache_get';
				$data	= call_user_func([$this, $method], $id);
		...
		$datas	= $this->get_values_by($non_cached_ids, $field);

		foreach($non_cached_ids as $id){
			$cache_value	= $datas[$id] ?? [];
			$cache_time		= $cache_value ? $this->cache_time : MINUTE_IN_SECONDS;
			$method			= $primary ? 'cache_set_by_primary_key' : 'cache_set';

			$cache_values[$id]	= $cache_value;

			call_user_func([$this, $method], $id, $cache_value, $cache_time);
		}
