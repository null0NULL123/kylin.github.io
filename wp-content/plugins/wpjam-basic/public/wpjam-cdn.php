...
class WPJAM_CDN{
	...
	public static function get_options(){
		return array_merge([''=>' '], wp_list_pluck(self::get_by(), 'title'));
	}
	...
	public static function filter_get_attachment_metadata($data, $id){
		if(wp_attachment_is_image($id) && empty($data['sizes'])){
			$img_url	= wp_get_attachment_url($id);
			$base_url	= str_replace(wp_basename($img_url), '', $img_url);
			...
			foreach(wp_get_registered_image_subsizes() as $name => $size){
				...
				if($downsize && !empty($downsize[3])){
					$data['sizes'][$name]	= [
						'file'			=> str_replace($base_url, '', $downsize[0]),
						'url'			=> $downsize[0],
						'height'		=> $downsize[2],
						'width'			=> $downsize[1],
...
class WPJAM_CDN_Setting{
	...
	public function get_sections(){
		...
		$cdn_fields		= [
			'cdn_name'		=> ['title'=>'云存储',	'type'=>'select',	'options'=>WPJAM_CDN::get_options(),	'class'=>'show-if-key'], 
		...
		$image_fields	= [
			...
			'webp'		=> ['title'=>'WebP 格式',	'type'=>'checkbox',	'description'=>'将图片转换成 WebP 格式。',	'show_if'=>self::get_show_if('IN', ['volc_imagex','aliyun_oss',	'qcloud_cos'])],
			...
			'wk_set'	=> ['title'=>'水印设置',		'type'=>'fieldset',	'show_if'=>self::get_show_if('!=','volc_imagex'),	'fields'=>[
				'view'		=> ['title'=>'使用说明：',	'type'=>'view',		'value'=>'请使用云存储域名下的图片，水印设置仅应用于文章内容中的图片'],
				'watermark'	=> ['title'=>'水印图片：',	'type'=>'image'],
				'disslove'	=> ['title'=>'透明度：',		'type'=>'number',	'class'=>'small-text',	'description'=>'1-100，默认100（不透明）', 'min'=>0, 'max'=>100],
				'gravity'	=> ['title'=>'水印位置：',	'type'=>'select',	'options'=>$watermark_options],
				'dx'		=> ['title'=>'水印边距：',	'type'=>'number',	'group'=>'gravity',	'class'=>'small-text',	'value'=>10],
				'view_d'	=> ['title'=>'',			'type'=>'view',		'group'=>'gravity',	'value'=>'<span class="dashicons dashicons-no-alt"></span>'],
				'dy'		=> ['title'=>'',			'type'=>'number',	'group'=>'gravity',	'class'=>'small-text',	'value'=>10],
				'width'		=> ['title'=>'最小尺寸：',	'type'=>'number',	'group'=>'size',	'show_if'=>self::get_show_if('IN', ['aliyun_oss', 'qcloud_cos']),		'name'=>'watermark_width',	'class'=>'small-text'],
				'view_s'	=> ['title'=>'',			'type'=>'view',		'group'=>'size',	'show_if'=>self::get_show_if('IN', ['aliyun_oss', 'qcloud_cos']),		'value'=>'<span class="dashicons dashicons-no-alt"></span>'],
				'height'	=> ['title'=>'',			'type'=>'number',	'group'=>'size',	'show_if'=>self::get_show_if('IN', ['aliyun_oss', 'qcloud_cos']),		'name'=>'watermark_height',	'class'=>'small-text',	'description'=>'小于该尺寸的图片都不会加上水印'],
			]],
...
add_action('plugins_loaded', function(){
	...
	if(CDN_NAME){
		...
		if(wpjam_cdn_get_setting('image', 1)){
			if($type_obj = WPJAM_CDN::get(CDN_NAME)){
				$type_obj->load();
			}
			...
			if(wpjam_cdn_get_setting('no_subsizes', 1)){
				...
				add_filter('embed_thumbnail_image_size',		['WPJAM_CDN', 'filter_embed_thumbnail_image_size']);
				// add_filter('wp_get_attachment_metadata',		['WPJAM_CDN', 'filter_get_attachment_metadata'], 10, 2);	// 附加上 sizes 数组会和很多插件冲突
