...
class WPJAM_SEO_Setting{
	...
	public function santize_callback($value){
		flush_rewrite_rules();
	...
	public function get_sections(){
		...
		$ptype_options	= wp_list_pluck(get_post_types(['show_ui'=>true,'public'=>true], 'objects'), 'label', 'name');
		$tax_options	= wp_list_pluck(get_taxonomies(['show_ui'=>true,'public'=>true], 'objects'), 'label', 'name');

		unset($ptype_options['attachment']);
		...
		return [
			'setting'	=> ['title'=>'SEO设置',	'fields'=>[
				...
				'individual_set'	=> ['title'=>'单独设置支持',	'type'=>'fieldset',	'show_if'=>['key'=>'individual', 'value'=>'1'],	'fields'=>[
					'post_types'	=> ['title'=>'文章类型',	'type'=>'checkbox',	'options'=>$ptype_options,	'value'=>['post']],
					'taxonomies'	=> ['title'=>'分类模式',	'type'=>'checkbox',	'options'=>$tax_options,	'value'=>['category']],
				]],
	...
	public static function get_meta_value($type='title'){
		...
		if(is_singular()){
			...
			if((is_home() || is_front_page()) && !wpjam_is_module()){
				$meta_value	= wpjam_seo_get_setting('home_'.$type);
			}elseif(is_tag() || is_category() || is_tax()){
				if(wpjam_seo_get_setting('individual') && get_queried_object()){
...
function wpjam_seo_get_setting($name, $default=null){
	return WPJAM_SEO_Setting::get_instance()->get_setting($name, $default);
}

add_action('init', function(){
	if(wpjam_seo_get_setting('sitemap') == 0){
		...
		wpjam_register_route_module('sitemap', ['callback'=>['WPJAM_SEO', 'sitemap']]);
	}

	add_action('wp_head',	['WPJAM_SEO', 'on_wp_head'], 0);

	add_filter('robots_txt',			['WPJAM_SEO', 'filter_robots_txt'], 10, 2);
	add_filter('document_title_parts',	['WPJAM_SEO', 'filter_document_title_parts']);
	add_filter('wpjam_post_json',		['WPJAM_SEO', 'filter_post_json'], 10, 2);
	add_filter('wpjam_posts_json',		['WPJAM_SEO', 'filter_posts_json']);
