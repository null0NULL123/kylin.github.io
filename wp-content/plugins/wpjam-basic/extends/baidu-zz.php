...
class WPJAM_Baidu_ZZ{
	public static function notify($urls, $args=[]){
		...
		$query_args['site']		= wpjam_baidu_zz_get_setting('site');
		$query_args['token']	= wpjam_baidu_zz_get_setting('token');
		...
		if(empty($args['type'])){
			if(wpjam_baidu_zz_get_setting('mip')){
				$query_args['type']	= 'mip';
			}
		}else{
...
function wpjam_baidu_zz_get_setting($name, $default=null){
	return WPJAM_Baidu_ZZ_Setting::get_instance()->get_setting($name, $default);
}
...
add_action('wp_loaded', function(){
	...
	if(!wpjam_baidu_zz_get_setting('no_js')){
	...
	if(is_admin()){
		...
		add_action('wpjam_builtin_page_load',	['WPJAM_Baidu_ZZ', 'on_builtin_page_load'], 10, 2);
	}
});
